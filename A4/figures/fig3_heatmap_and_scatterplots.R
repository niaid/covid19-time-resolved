...
#ESET_PARENT_IN_PATH <- "data/CITE5p/all_batches/differential_expression_cell_freq/2020_07_24/all_timepoints_covid_only/subsetted_expressionsets/WCT_course_pcnt_parent-eset.rds"
...
SCORES_IN_PATH <- "/hpcdata/sg/sg_data/users/lauwill/COVID/SeverityNetwork/output/healthy_vs_covid_COVID-Healthy_module_score_gsva_filtered_samples_genes.rds"
...
load("forSubmission/covid19.metadata.paper1.RData")
...
#pdc_samples_w_scores <- scores %>%
#        filter(pathway == "GO_APOPTOTIC_SIGNALING_PATHWAY" & celltype == "pDC") %>%
#        pull(sample) %>% grep(pattern = "T0", value = T)
#pdc_samples_w_scores
#
#pdc_dge <- readRDS("data/CITE5p/all_batches/differential_expression/2020_08_09/sample_groups/t0_covid_only/pseudobulk_dgelists/Unsorted-WCTcoursecelltype/pDC-pseudobulk_dgelist.rds")
#
#unoverlap <- setdiff(pdc_samples_w_scores, colnames(pdc_dge))
#unfiltered_list <- readRDS("data/CITE5p/all_batches/differential_expression/2020_08_09/sample_groups/t0_covid_only/pseudobulk_dgelists_unfiltered/Unsorted-WCTcoursecelltype.rds")
#
#pdc_unfilt <- unfiltered_list$pDC
#
#pdc_unfilt$samples$n_barcodes[colnames(pdc_unfilt) %in% unoverlap]
...
#pdf(out_path_list$EXPLORE, height = 10, width = 10)
#corrplot.mixed(cormat, main = "spearman correlation using pairwise complete observations\nIncluding healthy", tl.pos = "lt")
##pairs(feature_mat, cex.labels = .5)
#ggpairs(as.data.frame(mat), aes(color = ifelse(grepl("HGR", dat$sample), "COVID", "HC")))
#
#corrplot.mixed(cormat_covid_only, main = "pearson correlation using pairwise complete observations\nCOVID only", tl.pos = "lt")
##pairs(feature_mat, cex.labels = .5)
#ggpairs(as.data.frame(mat_covid_only))
#dev.off()


#diag(cormat_covid_only) <- NA
...
#plot_dat %>% filter(severity.outcome == "na")

#pdat %>% filter(Donor %in% c("HGR0000124",  "HGR0000079"))
#
#meta_w_endpts <- readRDS("data/metadata/endpoints/2020_08_09/batch.sample.end.points.log10.RDS")
#
#meta_w_endpts %>% filter(subject_id %in% c("HGR0000124",  "HGR0000079"))
