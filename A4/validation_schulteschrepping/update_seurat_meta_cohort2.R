...
IN_PATH <- "/hpcdata/sg/sg_data/users/martinsaj/COVID19/externalData/SchulteSchrepping/seurat_COVID19_PBMC_jonas_FG_2020-07-23.rds"

OUT_PATH <- "data/externalData/SchulteSchrepping/seurat_COVID19_PBMC_jonas_FG_add_timepoint_2020-11-4.rds"
#dir.create(dirname(OUT_PATH), recursive = TRUE)

LABEL_TRANSFER_PATH <- "/hpcdata/sg/sg_data/users/martinsaj/COVID19/dataupload/SeuratObjects/predictedLabels.Schulte.C2.Rds"
...
#orig.ident has two values per sample - guess there were 2 cartridges used
keep_cols <- c(
  #"orig.ident",
  "donor", "sampleID", "days_after_onset", 
  "experiment", "age", "sex", 
...
meta <- meta %>% mutate(onset_group = NA) %>%
        mutate(onset_group = replace(onset_group, days_after_onset < 8, "<8")) %>%
        mutate(onset_group = replace(onset_group, days_after_onset == 0, "HC")) %>%
        mutate(onset_group = replace(onset_group, days_after_onset >= 8 & days_after_onset <= 12, "8-12")) %>%
        mutate(onset_group = replace(onset_group, days_after_onset >= 13 & days_after_onset <= 16, "13-16")) %>%
        mutate(onset_group = replace(onset_group, days_after_onset >= 17 & days_after_onset <= 23, "17-23")) %>%
        mutate(onset_group = replace(onset_group, days_after_onset > 23, ">23"))
...
meta <- meta %>% mutate(onset_group2 = NA) %>%
        ...
        mutate(onset_group2 = replace(onset_group2, days_after_onset >= 7 & days_after_onset < 17, "prejuncture7_17")) %>%
        mutate(onset_group2 = replace(onset_group2, days_after_onset >= 17 & days_after_onset <= 23, "juncture17_23")) %>%
        mutate(onset_group2 = replace(onset_group2, days_after_onset > 23, "after23"))
