...
LIBSIZE_CUTOFF <- snakemake@params[["libsize_filter"]]
...
if(file_test("-d", IN_PATH)){
  ...
  names(pseudobulk_list) <- gsub("-pseudobulk_dgelist.rds", "", basename(files))
}else if(file_test("-f", IN_PATH)){
  pseudobulk_list <- readRDS(IN_PATH)
}

print(str(pseudobulk_list))

meta_list <- lapply(pseudobulk_list, `[[`, "samples")

meta_dat <- bind_rows(meta_list, .id = "celltype")

meta_dat <- meta_dat %>% 
        select(n_barcodes, celltype, lib.size, group_per_sample, sex)

meta_dat <- meta_dat %>%
        filter(!celltype %in% "ungated")

pdf(FIG_OUT_PATH)
p1 <- ggplot(meta_dat, aes(x = celltype, fill = group_per_sample)) +
        geom_bar() +
...
p1_gender <- ggplot(meta_dat, aes(x = celltype, fill = sex)) +
        geom_bar() +
        ggtitle("N samples per celltype") +
        theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
p2 <- ggplot(meta_dat, aes(x = celltype, y = log2(n_barcodes))) + 
        ...
        geom_jitter(aes(color = group_per_sample, shape = sex)) +
        ggtitle("Number of cells per sample") +
        theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
        geom_hline(yintercept = log2(MIN_CELLS_PER_SAMPLE), color = "red")
print(p2)

p3 <- ggplot(meta_dat, aes(x = celltype, y = log10(lib.size))) + 
        geom_boxplot(outlier.shape = NA) +
        geom_jitter(aes(color = group_per_sample, shape = sex)) +
        ggtitle("Library size per sample") +
        theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
        geom_hline(yintercept = log10(LIBSIZE_CUTOFF), color = "red")
print(p3)
dev.off()
