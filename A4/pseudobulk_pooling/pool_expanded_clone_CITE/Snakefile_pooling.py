workdir: "data/CITE5p/all_batches/expanded_tcell_pbulk_CITE_DE/2020_07_31/"

SORT_GROUPS = ["Sorted", "Unsorted"]

CELL_ANNOTATION = ["WCTcoursecelltype"]
...
rule all:
    input:
        expand("sample_groups/{sample_group}/pseudobulk_esets/{sort_group}-{cell_annotation}/", sample_group = SAMPLE_GROUPS, sort_group = SORT_GROUPS, cell_annotation = CELL_ANNOTATION),
...
rule pseudobulk_pool:
    input:
        seurat="data/CITE5p/all_batches/2020_07_24.rmBuffSHD8.allcelltypelabels.merge.SNG.wmeta.WithinBatchClustered.Rds",
        expanded_dat="data/CITE5p/all_batches/tcr/2020_07_28_tcell_expansion_df.tsv"
    output:
        "all_samples/pseudobulk_esets_unfiltered/{sort_group}-{cell_annotation}.rds"
    params:
        ...
        keep_celltypes = ["CD4_Mem", "CD8_Mem"],
...
rule subset_pseudobulk:
    input:
        "all_samples/pseudobulk_esets_unfiltered/{sort_group}-{cell_annotation}.rds"
    output:
        "sample_groups/{sample_group}/pseudobulk_esets_unfiltered/{sort_group}-{cell_annotation}.rds"
    params:
...
rule filter_pseudobulk_list:
    input:
        "sample_groups/{sample_group}/pseudobulk_esets_unfiltered/{sort_group}-{cell_annotation}.rds"
    output:
        directory("sample_groups/{sample_group}/pseudobulk_esets/{sort_group}-{cell_annotation}/")
    params:
...
rule plot_pseudobulk_stats_post_filter:
    input:
        "sample_groups/{sample_group}/pseudobulk_esets/{sort_group}-{cell_annotation}/"
    output:
