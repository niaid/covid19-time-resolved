...
load("covid19.metadata.paper1.RData")
...
covid19.samples.pbmc$severity_outcome <- paste(covid19.samples.pbmc$severity,
                                               ...
                                               sep = "_")
PC1 <- readRDS("citeseq.patient.end.points.RDS")
...
# plot 5 plots including Timepoint, severity, time_since_onset, PC1class and PC1 correlation
plot5cat <- function(df, name, ratioto, width = 26, height = 18){
  ...
  means <- plyr::ddply(tmp.HC, .(variable), summarise, median = median(value), 
  ...
  p3 <- ggplot(tmp)+
    ...
    stat_summary(aes(x = days_since_symptoms_onset, y = value, group = PC1class), fun.y = median)+
